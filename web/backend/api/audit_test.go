...
// TestAuditPDF: the endpoint streams a PDF inline, uncached, built from the
...
func TestAuditPDF(t *testing.T) {
	...
	mux.ServeHTTP(rr, httptest.NewRequest(http.MethodGet, "/api/audit/pdf", nil))
	...
	if cd := rr.Header().Get("Content-Disposition"); !strings.HasPrefix(cd, `inline; filename="claweh-audit-`) {
...
// TestAuditEnvironment: process facts are filled, never left empty.
func TestAuditEnvironment(t *testing.T) {
	env := auditEnvironment("/tmp/config.json", "/tmp/data", testNow)
