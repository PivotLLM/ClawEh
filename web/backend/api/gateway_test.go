...
func TestGatewayStartReady_NoDefaultModel(t *testing.T) {
	...
	// Write a config JSON that explicitly nullifies the model so DefaultConfig's
	// default model is not inherited when the file is loaded.
...
func TestGatewayStartReady_InvalidDefaultModel(t *testing.T) {
	...
	cfg.Agents.Defaults.SetDefaultModel("missing-model")
...
func TestGatewayStartReady_LocalModelWithoutAPIKey(t *testing.T) {
	...
	cfg.Agents.Defaults.SetDefaultModel("local-vllm")
	err = config.SaveConfig(configPath, cfg)
	if err != nil {
		t.Fatalf("SaveConfig() error = %v", err)
	}
...
func TestGatewayStartReady_RemoteVLLMWithAPIKeyDoesNotProbe(t *testing.T) {
	...
	cfg.Agents.Defaults.SetDefaultModel("remote-vllm")
	err = config.SaveConfig(configPath, cfg)
	if err != nil {
		t.Fatalf("SaveConfig() error = %v", err)
	}
...
func TestGatewayStartReady_LocalOllamaUsesDefaultProbeBase(t *testing.T) {
	...
	cfg.Agents.Defaults.SetDefaultModel("local-ollama")
	err = config.SaveConfig(configPath, cfg)
	if err != nil {
		t.Fatalf("SaveConfig() error = %v", err)
	}
...
func TestGatewayStartReady_OAuthModelRequiresStoredCredential(t *testing.T) {
	...
	cfg.ModelList = []config.ModelConfig{{
		ModelName:  "anthropic-oauth",
		Model:      "anthropic/claude-sonnet-4.6",
		AuthMethod: "token",
		Enabled:    true,
	}}
	cfg.Agents.Defaults.SetDefaultModel("anthropic-oauth")
	err = config.SaveConfig(configPath, cfg)
	if err != nil {
		t.Fatalf("SaveConfig() error = %v", err)
	}
	...
	err = auth.SetCredential(oauthProviderAnthropic, &auth.AuthCredential{
...
func TestFindClawBinary_EnvOverride(t *testing.T) {
	// Create a temporary file to act as the mock binary
	tmpDir := t.TempDir()
	mockBinary := filepath.Join(tmpDir, "claw-mock")
	if err := os.WriteFile(mockBinary, []byte("mock"), 0o755); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}

	t.Setenv("CLAW_BINARY", mockBinary)

	got := utils.FindClawBinary()
	if got != mockBinary {
		t.Errorf("FindClawBinary() = %q, want %q", got, mockBinary)
	}
}

func TestFindClawBinary_EnvOverride_InvalidPath(t *testing.T) {
	// When CLAW_BINARY points to a non-existent path, fall through to next strategy
	t.Setenv("CLAW_BINARY", "/nonexistent/claw-binary")

	got := utils.FindClawBinary()
	// Should not return the invalid path; falls back to "claw" or another found path
	if got == "/nonexistent/claw-binary" {
		t.Errorf("FindClawBinary() returned invalid env path %q, expected fallback", got)
	}
}
