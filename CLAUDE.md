...
- **Operator alerts**: `github.com/tenebris-tech/alerter` (queued, de-duplicated alerts; v0.0.x writes a log). The gateway builds one in `internal/gateway/alerts.go` (app name, short hostname, `<CLAW_HOME>/logs/alerts.log` unless `ALERTER_LOG` is set) and hands it to the agent loop, which passes it to the cooldown tracker and MCP manager; the channel manager and cron service get it too. Channels get it through `BaseChannel.SetAlerter`/`Alert` (injected by the manager). Every alert (title, event id, priority, source) is listed in `ALERTS.md`; **add a row there when you add an alert.** Rule of thumb: high = a person must act, nothing in ClawEh will recover; low = degraded but retrying. `GET /api/gateway/alerts` tails the file; the Logs page shows it via its source selector. See `docs/alerts.md`.
