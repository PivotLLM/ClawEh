...
- **Audit report**: the `audit` package builds a config-derived inventory of what the install can do (listeners, providers and models, tokens as set/not set, channels, per-agent tools and folder access, external services, devices, data, schedules) and renders it as PDF; `GET /api/audit/pdf` serves it and the WebUI Audit page opens it. It never emits a secret value (guarded by a test). See `docs/audit.md`.
