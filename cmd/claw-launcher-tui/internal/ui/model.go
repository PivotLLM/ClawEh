...
func (s *appState) modelMenu() tview.Primitive {
	...
	currentModel := strings.TrimSpace(s.config.Agents.Defaults.DefaultModelName())
	...
	items = append(items,
		MenuItem{
			...
			Action: func() {
				...
				s.addModel(
					clawconfig.ModelConfig{ModelName: newName, Model: "openai/gpt-5.4"},
				)
				s.push(
	...
	menu.SetInputCapture(func(event *tcell.EventKey) *tcell.EventKey {
		...
		if event.Rune() == ' ' {
			...
			if row >= 0 && row < len(s.config.ModelList) {
				...
				s.config.Agents.Defaults.SetDefaultModel(model.ModelName)
				s.dirty = true
				refreshModelMenu(menu, s.config.Agents.Defaults.DefaultModelName(), s.config.ModelList)
...
func (s *appState) modelForm(index int) tview.Primitive {
	...
	form.AddCheckbox("Enabled", model.Enabled, func(checked bool) {
	...
	addInput(form, "Model Name", model.ModelName, func(value string) {
		...
		if s.config.Agents.Defaults.DefaultModelName() == oldName {
			s.config.Agents.Defaults.SetDefaultModel(value)
		}
		s.dirty = true
...
func (s *appState) addModel(model clawconfig.ModelConfig) {
...
func refreshModelMenu(menu *Menu, currentModel string, models []clawconfig.ModelConfig) {
...
func (s *appState) testModel(model *clawconfig.ModelConfig) {
	if model == nil {
		return
	}
