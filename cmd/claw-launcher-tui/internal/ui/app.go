...
import (
	...
	configstore "github.com/PivotLLM/ClawEh/cmd/claw-launcher-tui/internal/config"
...
func refreshMainMenu(menu *Menu, s *appState) {
	...
	items := []MenuItem{
		...
		{
			Label:       "Start Talk",
			Description: "Open " + global.AppName + " agent in terminal",
			Action: func() {
				s.requestStartTalk()
			},
			Disabled: !modelReady,
...
func (s *appState) selectedModelName() string {
	modelName := strings.TrimSpace(s.config.Agents.Defaults.DefaultModelName())
	if modelName == "" {
		return ""
	}
