...
| `mcpserver` | | MCP host server stopped | ClawEh's own MCP server (the one external clients and CLI providers use for host tools) died after startup. The gateway shuts down and exits with status 3 so the service manager restarts it. Details carry the error. Source: `internal/gateway/fatal.go`. |
| `http` | | HTTP listener stopped | The loopback HTTP listener (IPv4 or IPv6) or the HTTPS listener died after start (a bind failure at start stops the gateway with an error instead). The gateway shuts down and exits with status 3 so the service manager restarts it. Details carry the address and error. Source: `internal/gateway/fatal.go`. |
| `agent-loop` | | Agent loop stopped | The agent loop returned an error; no inbound message can be processed, so the gateway shuts down and exits with status 3 for the service manager to restart it. Details carry the error. Source: `internal/gateway/fatal.go`. |
| `<channel name>` | | Channel failed to start | The channel's Start has failed `StartRetryAlertAfter` (ten, in `channels/tuning.go`) times in a row since the gateway started or reloaded. Retrying continues indefinitely at the backoff ceiling (`StartRetryMax`, five minutes) until it starts or the gateway stops; raised once per start. Details carry the latest start error; success is logged as "Channel started after retries". Source: `channels/manager.go`. |
...
| `<channel name>` | | Channel receive loop stopped | The device gateway listener failed. The channel still reports running and re-listens with backoff (`ConnRetryMin` to `ConnRetryMax`, two seconds to one minute), retrying a refused bind the same way; raised once per outage, and recovery is logged as "Device gateway listener restored". Details carry the error. Source: `channels/device`. |
...
| `<client IP>` | | Device authentication locked out | A client address failed device gateway authentication `DeviceAuthFailThreshold` (5) times inside `DeviceAuthFailWindow` (ten minutes, `channels/tuning.go`) and is locked out; connects from it are answered `AUTH_RATE_LIMITED` for one minute, doubling on each further lockout up to an hour. Raised on the first lockout for an address only; later lockouts for the same address are logged at warn. Description gives the address, the count and the lockout length. Source: `channels/device/server.go`. |
| `auth-lockout` | | WebUI login locked out | A client address exceeded five failed logins in ten minutes and is locked out (one minute, doubling on each repeat up to an hour). Raised on the first lockout per address; Description names the address, Details the last username tried. Source: `web/backend/api/auth.go`. |
| `spend:<yyyy-mm-dd>` | | Daily model spend over threshold | Raised once per UTC day, the first time the day's summed model cost (from the providers' dispatch status) reaches `agents.defaults.daily_spend_alert_usd`; off when that key is 0. Description gives the day, the running total and the threshold. Counts inbound, cron and system-message turns, not sub-agent workers or compaction calls. Source: `agent/dispatch_event.go`. |
| `backup:<archive path>` | | Database failed integrity check | A SQLite database (session archive, cognitive memory, device or fusion token store) failed `PRAGMA quick_check` during a backup and was left out of the archive; the rest of the backup was written. The store is probably corrupt: stop the gateway and inspect it, or restore it from an earlier archive. Details carry SQLite's report. Source: `internal/backup/backup.go`. |
| `session-retention` | | Session retention failed | The nightly pass could not delete one or more idle session archives or cogmem pre-migration snapshots (permissions, disk); whatever succeeded stays deleted and nothing retries before tomorrow night. Details carry the errors. Source: `agent/retention.go`. |
| `session-store` | | Session store write failed | A user message, assistant reply, tool call, tool result, or vision message could not be written to the session store, so the turn was abandoned with an error to the user instead of continuing on a history the store did not accept (disk full or unwritable?). Details carry the store error. Source: `agent/loop_turn.go`. |
| `<session key>` | | Context compaction breaker tripped | Three consecutive automatic compactions failed for the session, so the engine has suspended normal compaction; only the safety-net pass still runs and the context grows until then. Check the summarization model chain (cooldowns, credentials, refusals). Description gives the failure count and agent. Source: `agent/context_manager.go`. |
| `tls-reload` | | TLS certificate reload failed | The certificate or key file changed on disk (mtime, size or symlink target) but the new pair did not load: it does not parse, the key does not match, or the certificate has expired. The previous certificate keeps serving; raised once per broken file state and cleared by the next good pair. Description gives the certificate path. Source: `internal/tlscert/tlscert.go`. |
| `tls-expiry` | | TLS certificate expires soon | An operator-supplied certificate (`gateway.tls.cert_file`) is within 14 days, within 3 days, or past its expiry — one alert at each mark per loaded pair; a renewed pair resets them. Self-signed certificates renew themselves and never raise this. Source: `internal/tlscert/tlscert.go`. |
| `tls-selfsigned` | | Self-signed TLS certificate renewal failed | The daily check found the self-signed certificate within 30 days of expiry and could not regenerate it (typically `<CLAW_HOME>/tls` not writable). The old certificate keeps serving until it expires. Source: `internal/tlscert/tlscert.go`. |
