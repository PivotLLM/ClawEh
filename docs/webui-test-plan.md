...
| F3 | Under **MCP access**, tick the `fusion` checkbox and type `trello` in the **Prefixes** field beneath it (no `fusion` server configured: type `fusion, trello` in that field), wait ~2s | `mcp_tools: ["fusion","trello"]` persisted **and** `temperature` is still `0.77`. The second save must not clobber the first |
