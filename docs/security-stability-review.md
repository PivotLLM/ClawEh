...
| 2 | **No in-process TLS.** Edge termination (Cloudflare, nginx, Tailscale) is the only HTTPS path; `gateway.external_url` advertises `https://` without terminating it. | `gateway.tls.{enabled,cert_file,key_file}` with `GetCertificate` and mtime/fsnotify reload so ACME renewals are picked up live; a failed reload keeps the previous cert. Reverse-proxy path stays supported. See [Proposal: native HTTPS](#proposal-native-https). | **Closed** (0.6.0). Loopback stays HTTP; a non-loopback `gateway.host` serves HTTPS on `gateway.tls_port` (18443) with a self-signed or user-supplied certificate, hot-reloaded from disk. See `docs/tls.md`. |
