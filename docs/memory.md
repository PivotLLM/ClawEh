...
The background consolidation prompt can be overridden per agent. Each agent workspace holds a `COGMEM.md`, seeded from the shipped template the first time the workspace is populated and never overwritten afterwards. Edit it to change how that assistant consolidates — what it should be more or less willing to record, subject matter to leave alone — and it takes effect on the next run. Delete it and the built-in default is used again.
