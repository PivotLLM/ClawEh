	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(cfg *config.Config) {
				cfg.Agents.Defaults.SetDefaultModel("openrouter/auto")
		...
		{
			name: "deepseek model uses deepseek defaults",
			setup: func(cfg *config.Config) {
				cfg.Agents.Defaults.SetDefaultModel("deepseek/deepseek-chat")
		...
		{
			name: "nvidia model uses nvidia defaults",
			setup: func(cfg *config.Config) {
				cfg.Agents.Defaults.SetDefaultModel("nvidia/meta/llama-3.1-8b")
		...
		},
		{
			...
			setup: func(cfg *config.Config) {
				cfg.Agents.Defaults.SetDefaultModel("groq/llama-3.3-70b")
		...
		},
		{
			...
			setup: func(cfg *config.Config) {
				cfg.Agents.Defaults.SetDefaultModel("ollama/qwen2.5:14b")
		...
		},
		{
			...
			setup: func(cfg *config.Config) {
				cfg.Agents.Defaults.SetDefaultModel("moonshot/kimi-k2.5")
		...
		},
		{
			...
			setup: func(cfg *config.Config) {
				cfg.Agents.Defaults.SetDefaultModel("custom-model")
			},
...
func TestCreateProviderReturnsHTTPProviderForOpenRouter(t *testing.T) {
	cfg := config.DefaultConfig()
	cfg.Agents.Defaults.SetDefaultModel("test-openrouter")
...
func TestCreateProviderReturnsCodexCliProviderForCodexCode(t *testing.T) {
	cfg := config.DefaultConfig()
	cfg.Agents.Defaults.SetDefaultModel("test-codex")
	cfg.ModelList = []config.ModelConfig{
		{
...
func TestCreateProviderReturnsClaudeCliProviderForClaudeCli(t *testing.T) {
	cfg := config.DefaultConfig()
	cfg.Agents.Defaults.SetDefaultModel("test-claude-cli")
	cfg.ModelList = []config.ModelConfig{
		{
...
func TestCreateProviderReturnsClaudeProviderForAnthropicOAuth(t *testing.T) {
	...
	cfg := config.DefaultConfig()
	cfg.Agents.Defaults.SetDefaultModel("test-claude-oauth")
	cfg.ModelList = []config.ModelConfig{
		{
...
// TestResolveProviderSelection_DeepSeekNoKey verifies that a deepseek/ model
// with no DeepSeek API key and no OpenRouter API key returns an error rather
// than silently falling through to OpenRouter.
func TestResolveProviderSelection_DeepSeekNoKey(t *testing.T) {
	cfg := config.DefaultConfig()
	cfg.Agents.Defaults.SetDefaultModel("deepseek/deepseek-chat")
	...
	if err == nil {
		t.Fatal("expected error for deepseek model with no API key, got nil")
	}
}

// TestResolveProviderSelection_DeepSeekWithKey verifies that a deepseek/ model
// with a non-empty DeepSeek API key succeeds and returns the DeepSeek API base URL.
func TestResolveProviderSelection_DeepSeekWithKey(t *testing.T) {
	cfg := config.DefaultConfig()
	cfg.Agents.Defaults.SetDefaultModel("deepseek/deepseek-chat")
	cfg.Providers.DeepSeek.APIKey = "sk-deepseek-test-key"
	cfg.Providers.OpenRouter.APIKey = ""

	sel, err := resolveProviderSelection(cfg)
	if err != nil {
		t.Fatalf("resolveProviderSelection() unexpected error = %v", err)
	}
	if !strings.Contains(sel.apiBase, "deepseek.com") {
		t.Fatalf("apiBase = %q, want URL containing deepseek.com", sel.apiBase)
	}
}
