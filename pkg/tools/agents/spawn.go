...
var _ tools.AsyncExecutor = (*SpawnTool)(nil)
...
func (t *SpawnTool) ExecuteAsync(ctx context.Context, args map[string]any, cb tools.AsyncCallback) *tools.ToolResult {
	return t.execute(ctx, args, cb)
}

func (t *SpawnTool) execute(ctx context.Context, args map[string]any, cb tools.AsyncCallback) *tools.ToolResult {
	...
	// Check allowlist only for targeted spawns. Self-spawns (agentID == "")
	// are authorized by the tool being registered in the agent's registry.
	if t.allowlistCheck != nil && agentID != "" {
