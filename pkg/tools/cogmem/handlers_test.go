...
func TestCreateRememberGetUpdateRetire(t *testing.T) {
	...
	res := run(t, h["create_domain"], newCall(testSession, map[string]any{
	...
	res = run(t, h["remember"], newCall(testSession, map[string]any{
	...
	res = run(t, h["get_domain"], newCall(testSession, map[string]any{"id": domainID}))
	if res.IsError {
	...
	res = run(t, h["update_domain"], newCall(testSession, map[string]any{
	...
	res = run(t, h["retire_hook"], newCall(testSession, map[string]any{
	...
	res = run(t, h["search"], newCall(testSession, map[string]any{"query": "sky"}))
	if res.IsError {
...
func TestSearchActiveVsReview(t *testing.T) {
	h, _ := buildHandlers(t)
	res := run(t, h["create_domain"], newCall(testSession, map[string]any{"type": "project", "name": "p"}))
	domainID := extractID(t, res.ForLLM, "d")

	// active hook
	run(t, h["remember"], newCall(testSession, map[string]any{
	...
	res = run(t, h["search"], newCall(testSession, map[string]any{"query": "widget"}))
...
func TestForgetRetiresMatches(t *testing.T) {
	...
	res = run(t, h["forget"], newCall(testSession, map[string]any{"query": "forget me"}))
	if res.IsError {
	...
	res = run(t, h["search"], newCall(testSession, map[string]any{"query": "keep this"}))
...
func TestArchiveAndListDomains(t *testing.T) {
	h, _ := buildHandlers(t)
	res := run(t, h["create_domain"], newCall(testSession, map[string]any{"type": "project", "name": "Bob proj"}))
	domainID := extractID(t, res.ForLLM, "d")

	res = run(t, h["list_domains"], newCall(testSession, map[string]any{"status": "active"}))
	if !strings.Contains(res.ForLLM, domainID) {
		t.Fatalf("active list missing domain: %s", res.ForLLM)
	}

	if r := run(t, h["archive_domain"], newCall(testSession, map[string]any{"id": domainID})); r.IsError {
...
func TestEmptySessionErrors(t *testing.T) {
	h, _ := buildHandlers(t)
	for _, name := range []string{"get_domain", "search", "remember", "status", "create_domain"} {
