...
func NewCopyFileTool(workspace string, restrict bool, allowPaths ...[]*regexp.Regexp) *CopyFileTool {
	...
	return &CopyFileTool{fs: buildFs(workspace, restrict, patterns)}
}
...
func NewCopyFileToolWithMemoryRedirect(
	...
	return &CopyFileTool{fs: buildFsWithMemoryRedirect(workspace, restrict, patterns, memoryRoot)}
}
...
func (t *CopyFileTool) Execute(ctx context.Context, args map[string]any) *ToolResult {
	...
	if !ok || src == "" {
		return ErrorResult("source_path is required")
	}
	dst, ok := args["destination_path"].(string)
	if !ok || dst == "" {
		return ErrorResult("destination_path is required")
	}
	overwrite := getBoolArg(args, "overwrite", false)

	data, err := copyFileViaFs(t.fs, src, dst, overwrite)
	if err != nil {
...
func copyFileViaFs(fsys fileSystem, src, dst string, overwrite bool) ([]byte, error) {
	...
	if srcInfo.IsDir() {
		return nil, fmt.Errorf("source is a directory; copy_file copies files only: %s", src)
	}
