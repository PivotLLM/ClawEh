...
func TestShellTool_URLsNotBlocked(t *testing.T) {
	...
	// not as file system paths. Also includes bare-domain URLs (no scheme prefix).
	commands := []string{
		...
		// Bare-domain URLs (no scheme prefix) — curl accepts these
		`curl -s "wttr.in/K1V+1A8?m&T"`,
