...
func (t *SPITool) Execute(ctx context.Context, args map[string]any) *tools.ToolResult {
	if runtime.GOOS != "linux" {
		return tools.ErrorResult("SPI is only supported on Linux. This tool requires /dev/spidev* device files.")
	}
	...
	default:
		return tools.ErrorResult(fmt.Sprintf("unknown action: %s (valid: list, transfer, read)", action))
	}
}

// list finds available SPI devices by globbing /dev/spidev*
func (t *SPITool) list() *tools.ToolResult {
	matches, err := filepath.Glob("/dev/spidev*")
	if err != nil {
		return tools.ErrorResult(fmt.Sprintf("failed to scan for SPI devices: %v", err))
	}
	...
	return tools.SilentResult(fmt.Sprintf("Found %d SPI device(s):\n%s", len(devices), string(result)))
}
