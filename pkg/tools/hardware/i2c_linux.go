...
func (t *I2CTool) scan(args map[string]any) *tools.ToolResult {
	...
	if err != nil {
		return tools.ErrorResult(fmt.Sprintf("failed to open %s: %v (check permissions and i2c-dev module)", devPath, err))
	}
	...
	if errno != 0 {
		return tools.ErrorResult(fmt.Sprintf("failed to query I2C adapter capabilities on %s: %v", devPath, errno))
	}
	...
	if len(found) == 0 {
		return tools.SilentResult(fmt.Sprintf("No devices found on %s. Check wiring and pull-up resistors.", devPath))
	}
	...
	return tools.SilentResult(fmt.Sprintf("Scan of %s:\n%s", devPath, string(result)))
}
...
func (t *I2CTool) readDevice(args map[string]any) *tools.ToolResult {
	...
	if length < 1 || length > 256 {
		return tools.ErrorResult("length must be between 1 and 256")
	}

	devPath := fmt.Sprintf("/dev/i2c-%s", bus)
	fd, err := syscall.Open(devPath, syscall.O_RDWR, 0)
	if err != nil {
		return tools.ErrorResult(fmt.Sprintf("failed to open %s: %v", devPath, err))
	}
	defer syscall.Close(fd)
	...
	_, _, errno := syscall.Syscall(syscall.SYS_IOCTL, uintptr(fd), i2cSlave, uintptr(addr))
	if errno != 0 {
		return tools.ErrorResult(fmt.Sprintf("failed to set I2C address 0x%02x: %v", addr, errno))
	}
	...
	if regFloat, ok := args["register"].(float64); ok {
		reg := int(regFloat)
		if reg < 0 || reg > 255 {
			return tools.ErrorResult("register must be between 0x00 and 0xFF")
		}
		_, err = syscall.Write(fd, []byte{byte(reg)})
		if err != nil {
			return tools.ErrorResult(fmt.Sprintf("failed to write register 0x%02x: %v", reg, err))
		}
	}
	...
	if err != nil {
		return tools.ErrorResult(fmt.Sprintf("failed to read from device 0x%02x: %v", addr, err))
	}
	...
	return tools.SilentResult(string(result))
}
...
func (t *I2CTool) writeDevice(args map[string]any) *tools.ToolResult {
	...
	if !ok || len(dataRaw) == 0 {
		return tools.ErrorResult("data is required for write (array of byte values 0-255)")
	}
	if len(dataRaw) > 256 {
		return tools.ErrorResult("data too long: maximum 256 bytes per I2C transaction")
	}
	...
	for i, v := range dataRaw {
		f, ok := v.(float64)
		if !ok {
			return tools.ErrorResult(fmt.Sprintf("data[%d] is not a valid byte value", i))
		}
		b := int(f)
		if b < 0 || b > 255 {
			return tools.ErrorResult(fmt.Sprintf("data[%d] = %d is out of byte range (0-255)", i, b))
		}
	...
	if err != nil {
		return tools.ErrorResult(fmt.Sprintf("failed to write to device 0x%02x: %v", addr, err))
	}

	return tools.SilentResult(fmt.Sprintf("Wrote %d byte(s) to device 0x%02x on %s", n, addr, devPath))
}
