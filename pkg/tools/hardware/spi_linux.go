...
func configureSPI(devPath string, mode uint8, bits uint8, speed uint32) (int, *tools.ToolResult) {
	fd, err := syscall.Open(devPath, syscall.O_RDWR, 0)
	if err != nil {
		return -1, tools.ErrorResult(fmt.Sprintf("failed to open %s: %v (check permissions and spidev module)", devPath, err))
	}
	...
	if errno != 0 {
		syscall.Close(fd)
		return -1, tools.ErrorResult(fmt.Sprintf("failed to set SPI mode %d: %v", mode, errno))
	}
	...
	if errno != 0 {
		syscall.Close(fd)
		return -1, tools.ErrorResult(fmt.Sprintf("failed to set bits per word %d: %v", bits, errno))
	}
	...
	if errno != 0 {
		syscall.Close(fd)
		return -1, tools.ErrorResult(fmt.Sprintf("failed to set SPI speed %d Hz: %v", speed, errno))
	}
...
func (t *SPITool) transfer(args map[string]any) *tools.ToolResult {
	...
	if !ok || len(dataRaw) == 0 {
		return tools.ErrorResult("data is required for transfer (array of byte values 0-255)")
	}
	if len(dataRaw) > 4096 {
		return tools.ErrorResult("data too long: maximum 4096 bytes per SPI transfer")
	}
	...
	if errno != 0 {
		return tools.ErrorResult(fmt.Sprintf("SPI transfer failed: %v", errno))
	}
...
func (t *SPITool) readDevice(args map[string]any) *tools.ToolResult {
	...
	if length < 1 || length > 4096 {
		return tools.ErrorResult("length is required for read (1-4096)")
	}
	...
	runtime.KeepAlive(rxBuf)
	if errno != 0 {
		return tools.ErrorResult(fmt.Sprintf("SPI read failed: %v", errno))
	}
