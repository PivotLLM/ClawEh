...
func (t *I2CTool) Execute(ctx context.Context, args map[string]any) *tools.ToolResult {
	if runtime.GOOS != "linux" {
		return tools.ErrorResult("I2C is only supported on Linux. This tool requires /dev/i2c-* device files.")
	}
	...
	default:
		return tools.ErrorResult(fmt.Sprintf("unknown action: %s (valid: detect, scan, read, write)", action))
	}
}

// detect lists available I2C buses by globbing /dev/i2c-*
func (t *I2CTool) detect() *tools.ToolResult {
	matches, err := filepath.Glob("/dev/i2c-*")
	if err != nil {
		return tools.ErrorResult(fmt.Sprintf("failed to scan for I2C buses: %v", err))
	}
	...
	return tools.SilentResult(fmt.Sprintf("Found %d I2C bus(es):\n%s", len(buses), string(result)))
}
...
//
//nolint:unused // Used by i2c_linux.go
func parseI2CAddress(args map[string]any) (int, *tools.ToolResult) {
	addrFloat, ok := args["address"].(float64)
	if !ok {
		return 0, tools.ErrorResult("address is required (e.g. 0x38 for AHT20)")
	}
	addr := int(addrFloat)
	if addr < 0x03 || addr > 0x77 {
		return 0, tools.ErrorResult("address must be in valid 7-bit range (0x03-0x77)")
	}
	return addr, nil
}
...
//
//nolint:unused // Used by i2c_linux.go
func parseI2CBus(args map[string]any) (string, *tools.ToolResult) {
	bus, ok := args["bus"].(string)
	if !ok || bus == "" {
		return "", tools.ErrorResult("bus is required (e.g. \"1\" for /dev/i2c-1)")
	}
	if !isValidBusID(bus) {
		return "", tools.ErrorResult("invalid bus identifier: must be a number (e.g. \"1\")")
	}
	return bus, nil
}
