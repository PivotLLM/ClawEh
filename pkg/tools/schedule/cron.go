...
func NewCronTool(
	...
	execTool, err := shell.NewExecToolWithConfig(workspace, restrict, config)
	if err != nil {
...
func (t *CronTool) addJob(ctx context.Context, args map[string]any) *tools.ToolResult {
	...
	if command != "" {
		if !constants.IsInternalChannel(channel) {
			return tools.ErrorResult("scheduling command execution is restricted to internal channels")
		}
		if !commandConfirm {
			return tools.ErrorResult("command_confirm=true is required to schedule command execution")
		}
		mode = "command"
	}
