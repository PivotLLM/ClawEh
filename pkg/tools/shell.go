...
// isHostnameChar reports whether b can appear in a hostname/domain,
// indicating the '/' after it is a URL path separator, not an absolute path.
...
func NewExecToolWithConfig(workingDir string, restrict bool, config *config.Config) (*ExecTool, error) {
	...
	if config != nil {
		...
		if enableDenyPatterns {
			...
			logger.WarnC("shell", "Deny patterns are disabled - all commands will be allowed")
		}
...
func (t *ExecTool) guardCommand(command, cwd string) string {
	...
	if t.restrictToWorkspace {
		...
		for _, loc := range matchIndices {
			...
			// Skip path components that follow a hostname character — these are
			// part of bare URLs like "wttr.in/K1V" where '/' is not an absolute
			// path start.
			if loc[0] > 0 {
				prev := cmd[loc[0]-1]
				if isHostnameChar(prev) {
					continue
				}
			}
