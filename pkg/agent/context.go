...
3. **Memory** - Record durable memory with the cogmem_* tools. You have one always-on **general** domain that is included in EVERY prompt — store critical global rules, preferences, and standing facts there with cogmem_hook_create (no domain argument), and they will always be in your context. Keep project-specific memory in a project domain. Your workspace .md files are read-only — do not edit them.
