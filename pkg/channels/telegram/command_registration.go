...
func commandRegistrationDelay(backoff []time.Duration, attempt int) time.Duration {
	if len(backoff) == 0 {
		return 0
	}
...
func (c *TelegramChannel) startCommandRegistration(ctx context.Context, defs []commands.Definition) {
	...
	// Capture the backoff schedule at startup so the goroutine holds its own copy
	// and does not race with tests that swap the package-level variable.
	backoff := commandRegistrationBackoff
	...
	go func() {
		...
		for {
			...
			delay := commandRegistrationDelay(backoff, attempt)
