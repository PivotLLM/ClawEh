...
func TestBuildAgentPeerSessionKey_SessionScopeUnified(t *testing.T) {
	...
	if got != want {
		t.Errorf("SessionScopeUnified = %q, want %q", got, want)
	}
}

func TestBuildAgentPeerSessionKey_ChannelPeerSessionScopeUnified(t *testing.T) {
	got := BuildAgentPeerSessionKey(SessionKeyParams{
		AgentID:      "amber",
		Channel:      "telegram",
		Peer:         &RoutePeer{Kind: "channel", ID: "chan789"},
		SessionScope: SessionScopeUnified,
	})
	want := "agent:amber:main"
	if got != want {
		t.Errorf("ChannelPeer SessionScopeUnified = %q, want %q", got, want)
	}
}

func TestBuildAgentPeerSessionKey_SessionScopePerUser(t *testing.T) {
	got := BuildAgentPeerSessionKey(SessionKeyParams{
		AgentID:      "main",
		Channel:      "telegram",
		Peer:         &RoutePeer{Kind: "direct", ID: "user123"},
		SessionScope: SessionScopePerUser,
	})
	want := "agent:main:direct:user123"
	if got != want {
		t.Errorf("SessionScopePerUser = %q, want %q", got, want)
	}
}

func TestBuildAgentPeerSessionKey_SessionScopePerPlatform(t *testing.T) {
	got := BuildAgentPeerSessionKey(SessionKeyParams{
		AgentID:      "main",
		Channel:      "telegram",
		Peer:         &RoutePeer{Kind: "direct", ID: "user123"},
		SessionScope: SessionScopePerPlatform,
	})
	want := "agent:main:telegram:direct:user123"
	if got != want {
		t.Errorf("SessionScopePerPlatform = %q, want %q", got, want)
	}
}

func TestBuildAgentPeerSessionKey_SessionScopePerAccount(t *testing.T) {
	got := BuildAgentPeerSessionKey(SessionKeyParams{
		AgentID:      "main",
		Channel:      "telegram",
		AccountID:    "bot1",
		Peer:         &RoutePeer{Kind: "direct", ID: "User123"},
		SessionScope: SessionScopePerAccount,
	})
	want := "agent:main:telegram:bot1:direct:user123"
	if got != want {
		t.Errorf("SessionScopePerAccount = %q, want %q", got, want)
	}
}
