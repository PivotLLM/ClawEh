...
func TestDispatch_SuccessEmitsAuthorizedInfoPerCall(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		out, isErr := dispatchToolCall(context.Background(), "read_file",
			map[string]any{"agent_token": tok, "path": "x"}, tm, nil, resolverFor(regs), tracker, nil)
		if isErr {
...
func TestDispatch_InvalidTokenEmitsWarn(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			buf.Reset()
			out, isErr := dispatchToolCall(context.Background(), "read_file",
				map[string]any{"agent_token": c.tok}, tm, nil, resolverFor(nil), nil, nil)
			if !isErr {
				t.Fatalf("expected rejection, got: %s", out)
			}
			logs := buf.String()
...
func TestDispatch_NoRegistryEmitsWarn(t *testing.T) {
	...
	out, isErr := dispatchToolCall(context.Background(), "read_file",
		map[string]any{"agent_token": tok}, tm, nil, resolverFor(nil), nil, nil)
	if !isErr {
		t.Fatalf("expected rejection, got: %s", out)
	}
	logs := buf.String()
