...
func TestDispatch_ValidTokenRoutesToAgentRegistry(t *testing.T) {
	...
	out, isErr := dispatchToolCall(context.Background(), "read_file",
		map[string]any{"agent_token": aliceTok, "path": "x"}, tm, nil, resolverFor(regs), nil, nil)
	if isErr {
	...
	out, isErr = dispatchToolCall(context.Background(), "read_file",
		map[string]any{"agent_token": bobTok, "path": "x"}, tm, nil, resolverFor(regs), nil, nil)
	if isErr {
		t.Fatalf("unexpected error: %s", out)
	}
	if out != "bob-content" {
...
func TestDispatch_EmptyTokenRejected(t *testing.T) {
	...
	tm.Issue("alice")
	out, isErr := dispatchToolCall(context.Background(), "read_file",
		map[string]any{}, tm, nil, resolverFor(nil), nil, nil)
	if !isErr {
...
func TestDispatch_UnknownTokenRejected(t *testing.T) {
	...
	out, isErr := dispatchToolCall(context.Background(), "read_file",
		map[string]any{"agent_token": bogus}, tm, nil, resolverFor(nil), nil, nil)
	if !isErr {
...
func TestDispatch_MalformedTokenRejected(t *testing.T) {
	...
	for _, c := range cases {
		out, isErr := dispatchToolCall(context.Background(), "read_file",
			map[string]any{"agent_token": c}, tm, nil, resolverFor(nil), nil, nil)
		if !isErr {
...
func TestDispatch_SubagentSentinelReturnsHelpfulError(t *testing.T) {
	...
	tm.Issue("alice")
	out, isErr := dispatchToolCall(context.Background(), "read_file",
		map[string]any{"agent_token": agenttoken.SubagentSentinel}, tm, nil, resolverFor(nil), nil, nil)
	if !isErr {
...
func TestDispatch_RelativePathOutsideWorkspaceRejected(t *testing.T) {
	...
	out, isErr := dispatchToolCall(context.Background(), "read_file",
		...
		tm, nil, resolverFor(map[string]*tools.ToolRegistry{"alice": reg}), nil, nil)
	if !isErr {
