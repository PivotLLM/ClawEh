...
//   - The memory templates are seeded into <workspace>/memory, and only for a
//     brand-new workspace, so a deleted memory directory does not reappear.
func Populate(workspace string) {
	...
	err := fs.WalkDir(templates.FS, ".", func(path string, d fs.DirEntry, err error) error {
		...
		// Memory templates are seeded separately (only for a brand-new
		// workspace) so a deleted memory directory does not reappear.
	...
	// Seed the starter memory only for a brand-new workspace. Skipping this on
	// an already-initialized workspace is what prevents a deleted memory
	// directory from reappearing.
	if !initialized {
		seedMemory(workspace)
	}
}

// seedMemory copies the embedded memory templates into <workspace>/memory,
// skipping any file that already exists.
func seedMemory(workspace string) {
