...
| D9 | Forget/purge | `cogmem_memory_forget` retires active memory + regenerates the read-only export. Physical archive purge stays an operator workflow, never an LLM tool. |
...
**Search at this scale.** `cogmem_memory_search` runs a plain SQL `LIKE`/scan over active
...
- **Proposed** - the sleep cycle may create a domain in **`review` status**, kept
  out of prompts until confirmed by later evidence or an explicit `cogmem_memory_create`
...
- **Pending confirmations.** `review` items are not silently shelved: a capped
  **pending digest** is surfaced in the prompt (§8/§10) so the agent can confirm
  ...
  batch, do not pester." On "yes" the agent calls `cogmem_memory_create` and the item
...
| `cogmem_memory_search` | read | Keyword/substring search (SQL scan) over active hooks. |
...
| `cogmem_memory_create` | write | Add/update a durable hook from explicit instruction or strong evidence. |
| `cogmem_domain_update` | write | Typed patch (summary/blockers/next-actions/constraints) by id with `expected_version`. |
| `cogmem_memory_retire` | write | Retire a hook by id with a reason (stays audited). |
...
| `cogmem_memory_forget` | write | Retire active memory for a topic; regenerate export (D9). |
...
concurrency). `cogmem_memory_forget` retires matching hooks and regenerates the export;
...
- `cogmem_memory_forget` lets the user remove active memory without a human file edit.
...
**Tools** (extend `tests/test_mcpserver.sh`): `cogmem_domain_get`/`cogmem_memory_search`
empty + positive; `cogmem_domain_create` returns an id; `cogmem_memory_create` creates
a hook with evidence; `cogmem_domain_update` rejects stale `expected_version`;
`cogmem_memory_retire` removes a hook from Compose; `cogmem_memory_forget` retires matches;
