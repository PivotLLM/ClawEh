...
// (~/.claw/skills) invalidates the cached system prompt.
...
// Run with: go test -race ./agent/ -run TestConcurrentBuildSystemPromptWithCache
	...
	for g := range goroutines {
		...
		go func(id int) {
			...
			for i := range iterations {
				...
				if !strings.Contains(result, "claw") {
